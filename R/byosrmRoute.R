...
#' @description This function add de distance and duration of the Shortest Path Between Two Points. Its use the osrmRoute function of osrm library, that build and send an OSRM API query to get the travel geometry between two points. 
...
#' @param src_dst Dataframe with four variables: x of source, y of source, x of destination, y of destination. x and y are plain coordinates in the defined CRS
...
byosrmRoute <- function(src_dst, crs){
        ...
        for (i in 1:nrow(pop1)) {
                r[[i]] <- osrmRoute(src = pop1[i, ], dst = centers1[i, ], overview = "full", returnclass = "sp")
